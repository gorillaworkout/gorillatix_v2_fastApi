...
export async function processTicketPurchase(formData: FormData) {
  try {
    ...
    const customerName = formData.get("customerName") as string;
    const venue = formData.get("venue") as string;
    const status = formData.get("status") as StatusTicketProps;
    ...
    if (ticket) {
      return { success: true, message: "Ticket purchased", ticketId: ticket.id };
    }

    return { success: false, message: "Ticket purchase failed." };
  } catch (error) {
    return {
      success: false,
